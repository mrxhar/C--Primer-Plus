...
int main()
{
    ...
    streampos place = rec * sizeof p1; // 定位到指定记录
    finout.seekg(place);
    if (finout.fail())
    {
    ...
    finout.read((char *)&p1, sizeof p1); // 读取指定记录
    ...
    finout.seekp(place);
    finout.write((char *)&p1, sizeof p1) << flush; // 刷新缓冲区
    if (finout.fail())
    {
